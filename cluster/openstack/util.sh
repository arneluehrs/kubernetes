...
find-object-url() {
  ...
  MY_KEY="secret"
  swift --os-auth-url ${OS_AUTH_URL} --os-username ${OS_USERNAME} --os-password ${OS_PASSWORD} post -m Temp-URL-Key:${MY_KEY}
...
${MINION_NAMES[$i]}"
    ...
    while [ ! "${MINION_VM_STATUS}" == "ACTIVE" ]; do
      MASTER_VM_STATUS=$(nova show ${MINION_NAMES[$i]} | grep status | awk '{ print $4}')
      printf "."
      sleep 2
    done
